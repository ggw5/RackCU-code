...
void send_data(TRANSMIT_DATA *td, char *server_ip, int port_num, ACK_DATA* ack, CMD_DATA* cmd, int send_data_type){
	...
	{
		//printf("port:%d\n",port_num);
		x=connect(client_socket,(struct sockaddr*)&server_addr, sizeof(server_addr));
		//printf("connect :x=%d\n",x);
		port_num++;
		if(port_num-port>10)
		{
...
void* recv_data_process(void* ptr){
    ...
    // copy the delta and record the data id 在这里保存多个data delta
...
void connect_metaserv(int chunk_id, META_INFO* metadata, int end){
	...
	//printf("send update req to mds\n");
    send_req(req, mt_svr_ip, UPDT_PORT, NULL, REQ_INFO);
	//printf("send update req to mds finish\n");
